...
public:
    ...
    void test(int matrixSize, int nbThreads, int nbBlocksPerRow)
    {
        ...
        for (int i = 0; i < nbThreadsForTester; i++) {
            std::thread* insiderThread = new std::thread(
                    test_int<ThreadedMultiplierType>, matrixSize, nbBlocksPerRow, threadedMultiplier.get());

            threadList.push_back(std::unique_ptr<std::thread>(insiderThread));
        }
